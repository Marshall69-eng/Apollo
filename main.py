...
#
# These are engineering safety constants, not user preferences. They
# are deliberately defined in code and exposed read-only over the API
# so that no dashboard operator can raise a threshold (and silence a
# real flood warning) during an event. Changing them is a code change
# that goes through review and redeployment.
#
# Calibration basis: MMDA/EFCOS Marikina River alarm practice and the
# 0.5-3.0 m/s working band used by the siting model above.
# ---------------------------------------------------------------
THRESHOLDS = {
    "enabled": True,
    "flood_critical": 85.0,      # flood-proximity index 0-100
    "flood_warning": 72.0,
...
def init_db() -> None:
    ...
    # Older prototype databases carried a writable `config` table of
    # thresholds. Thresholds are locked in code now, so drop it rather
    # than leave a stale, ignored copy that could mislead an operator.
...
def read_config() -> Dict[str, Any]:
    """Return the locked thresholds. Read-only by design — there is no
    setter anywhere in the API surface."""
...
def get_alert_config():
    """Read-only view of the locked alert thresholds.

    There is intentionally no PUT/PATCH counterpart: thresholds are
    safety constants set in code, so a dashboard user cannot raise a
    limit and suppress a warning while an event is in progress.
    """
    return {**read_config(), "channels": alert_service.channel_status()}
